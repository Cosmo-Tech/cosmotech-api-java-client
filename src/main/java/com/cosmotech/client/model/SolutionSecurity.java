...
public class SolutionSecurity {
  ...
  @com.redis.om.spring.annotations.Indexed
  private List<SolutionAccessControl> accessControlList = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to SolutionSecurity
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!SolutionSecurity.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in SolutionSecurity is not found in the empty JSON string", SolutionSecurity.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!SolutionSecurity.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `SolutionSecurity` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : SolutionSecurity.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayaccessControlList.size(); i++) {
        SolutionAccessControl.validateJsonElement(jsonArrayaccessControlList.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!SolutionSecurity.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'SolutionSecurity' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<SolutionSecurity> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(SolutionSecurity.class));

       return (TypeAdapter<T>) new TypeAdapter<SolutionSecurity>() {
           @Override
           public void write(JsonWriter out, SolutionSecurity value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public SolutionSecurity read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of SolutionSecurity given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of SolutionSecurity
  * @throws IOException if the JSON string is invalid with respect to SolutionSecurity
  */
  public static SolutionSecurity fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, SolutionSecurity.class);
  }

 /**
  * Convert an instance of SolutionSecurity to an JSON string
  *
  * @return JSON string
  */
