...
public class DatasetSecurity {
  ...
  @com.redis.om.spring.annotations.Indexed
  private List<DatasetAccessControl> accessControlList = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to DatasetSecurity
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!DatasetSecurity.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in DatasetSecurity is not found in the empty JSON string", DatasetSecurity.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!DatasetSecurity.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `DatasetSecurity` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : DatasetSecurity.openapiRequiredFields) {
      ...
      for (int i = 0; i < jsonArrayaccessControlList.size(); i++) {
        DatasetAccessControl.validateJsonElement(jsonArrayaccessControlList.get(i));
      };
  }
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!DatasetSecurity.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'DatasetSecurity' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<DatasetSecurity> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(DatasetSecurity.class));

       return (TypeAdapter<T>) new TypeAdapter<DatasetSecurity>() {
           @Override
           public void write(JsonWriter out, DatasetSecurity value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public DatasetSecurity read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of DatasetSecurity given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of DatasetSecurity
  * @throws IOException if the JSON string is invalid with respect to DatasetSecurity
  */
  public static DatasetSecurity fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, DatasetSecurity.class);
  }

 /**
  * Convert an instance of DatasetSecurity to an JSON string
  *
  * @return JSON string
  */
