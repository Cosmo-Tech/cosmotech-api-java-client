...
public class ScenarioRunStartContainers {
  ...
  private List<ScenarioRunContainer> containers = new ArrayList<>();
  ...
  * @throws IOException if the JSON Element is invalid with respect to ScenarioRunStartContainers
  */
  public static void validateJsonElement(JsonElement jsonElement) throws IOException {
      if (jsonElement == null) {
        if (!ScenarioRunStartContainers.openapiRequiredFields.isEmpty()) { // has required fields but JSON element is null
          throw new IllegalArgumentException(String.format("The required field(s) %s in ScenarioRunStartContainers is not found in the empty JSON string", ScenarioRunStartContainers.openapiRequiredFields.toString()));
        }
      }
      ...
      for (Map.Entry<String, JsonElement> entry : entries) {
        if (!ScenarioRunStartContainers.openapiFields.contains(entry.getKey())) {
          throw new IllegalArgumentException(String.format("The field `%s` in the JSON string is not defined in the `ScenarioRunStartContainers` properties. JSON: %s", entry.getKey(), jsonElement.toString()));
        }
      }

      // check to make sure all required properties/fields are present in the JSON string
      for (String requiredField : ScenarioRunStartContainers.openapiRequiredFields) {
  ...
  public static class CustomTypeAdapterFactory implements TypeAdapterFactory {
    ...
    public <T> TypeAdapter<T> create(Gson gson, TypeToken<T> type) {
       if (!ScenarioRunStartContainers.class.isAssignableFrom(type.getRawType())) {
         return null; // this class only serializes 'ScenarioRunStartContainers' and its subtypes
       }
       final TypeAdapter<JsonElement> elementAdapter = gson.getAdapter(JsonElement.class);
       final TypeAdapter<ScenarioRunStartContainers> thisAdapter
                        = gson.getDelegateAdapter(this, TypeToken.get(ScenarioRunStartContainers.class));

       return (TypeAdapter<T>) new TypeAdapter<ScenarioRunStartContainers>() {
           @Override
           public void write(JsonWriter out, ScenarioRunStartContainers value) throws IOException {
             JsonObject obj = thisAdapter.toJsonTree(value).getAsJsonObject();
             elementAdapter.write(out, obj);
           }

           @Override
           public ScenarioRunStartContainers read(JsonReader in) throws IOException {
 ...
 /**
  * Create an instance of ScenarioRunStartContainers given an JSON string
  *
  * @param jsonString JSON string
  * @return An instance of ScenarioRunStartContainers
  * @throws IOException if the JSON string is invalid with respect to ScenarioRunStartContainers
  */
  public static ScenarioRunStartContainers fromJson(String jsonString) throws IOException {
    return JSON.getGson().fromJson(jsonString, ScenarioRunStartContainers.class);
  }

 /**
  * Convert an instance of ScenarioRunStartContainers to an JSON string
  *
  * @return JSON string
  */
